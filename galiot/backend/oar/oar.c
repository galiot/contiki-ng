...
PROCESS(webserver_process,  "webserver process" );
            ...
            int rcrd = 0; // type 0 json is quantized, it's structured to be sent in pieces (so that they fit inside a tcp/ip packet). This variable is the record. Every record has 24 parts, indexed by index (see declaration below).
            int ndx = 0; // type 0 json is quantized, it's structured to be sent in pieces (so that they fit inside a tcp/ip packet). This variable is the index. It takes values from 0 to 24 inclusive.
...
static PT_THREAD(generate_routes(struct httpd_state *s))
{
    ...
    // <><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><
        ...
        else // if heap memory could be allocated for decrypted_oar_json...
        {
            ...
            // printf("%s\n", encrypted_oar_json); // printing non-printable charecters on terminal is a terrible idea - printouts cannot be copied/pasted safely (i.e. Microsoft Word crashes if such printout is included)
            for (int k = 0; k < strlen(encrypted_oar_json); k++) 
            {
                printf("*");
            }
    ...
    tcpip_event_counter %= 24999; // reset the counter, protect from overflow, 1000 records of 24 indexed seem enough for monitoring

    if (tcpip_event_counter % 25 == 0) { rcrd++; } // every 24 TCP/IP events, increase the record by one (24 parts, the complete json, must have been sent)
    ndx = tcpip_event_counter % 25; // reset index every 24 TCP/IP events. Send a unique part in every iteration of the tcpip_event_counter.
